import {CurrentUser} from './CurrentUser';
import {AddressType, UsersObjectType} from './HW2';

export type AddressTypeProps = {
	street: string;
	city: string;
};
type UsersObjectTypeProps = {
	id: number;
	name: string;
	age: number;
	address: AddressTypeProps;
}

type FriendsUserType = {
	myFriends: Array<UsersObjectTypeProps>

}

type UserList2PropsType = {
	users: FriendsUserType // пропиши типизацию
	filterUsers: () => void // пропиши типизацию
};

export const UserList2 = (props: UserList2PropsType) => {
	console.log(props)

	return (
		<div id={'hw02-users'}>
			<h2>User List:</h2>

			<button id={'hw02-filter-button'} onClick={() => props.filterUsers()}>SHOW ME FRIENDS FROM LA</button>

			<ul>
			  {props.users.myFriends.map((user:any) => (
			    <CurrentUser user={user}/>
			  ))}
			</ul>
		</div>
	);
};
